...
ACE_BEGIN_VERSIONED_NAMESPACE_DECL
  ...
  class isaMessager
  {
	...
	void init(const ACE_TCHAR* name, bool outInFile=false)
	{
		ACE_TCHAR defname[] = ACE_TEXT("ISA_VM");
		const ACE_TCHAR* pname = (name == NULL) ? defname : name;
		
		if ( outInFile) {
			log.open(pname,ACE_Log_Msg::OSTREAM);
 			//ACE_CString  logfilename = name + ".log";
 			ofstream *logostream=new ofstream(pname, ios::out);
  			// Check for errors.
		 	if (!logostream->bad ()){
			  // Set the ostream.
		...
		log.file(ACE_TEXT_ALWAYS_CHAR(pname));
	}
	/*void init(ACE_CString name="ISA_VM",bool outInFile=false)
	{
		if ( outInFile) {
